...
- The image embedded in `technical-roadmap` must be the actual technical flowchart / architecture diagram / method overview figure. Never embed a full PDF page, full paper screenshot, browser viewport, or combined page render as the roadmap image.
- `technical-roadmap` must contain exactly one main roadmap image. Put other figures/tables in `section#figures`, not in the roadmap section.
- Every embedded paper figure/table image must be clickable to enlarge in-page, then close when the enlarged image, backdrop, close button, or Escape key is clicked. Use the template's `image-lightbox` / `image-zoom` behavior; do not rely on opening a new browser tab.
- Put the roadmap image inside `<figure class="figure-panel roadmap-figure">`. Put other paper figures/tables inside `<figure class="figure-panel">`. The in-page preview should stay compact while the lightbox lets the user inspect details.
...
6. Write the HTML report.
   ...
   - Embed exactly one main technical-route figure directly in the technical-roadmap section and explain each visible component of the figure. This must be a tight crop of the figure itself, not a page render. If a tight crop cannot be created, omit the image and explain the limitation in prose.
   - Use this structure for the roadmap image so it is compact on the page and automatically uses the template lightbox:
     `<figure class="figure-panel roadmap-figure"><img src="sources/<paper-slug>/figures/<roadmap-crop>.png" alt="...">...</figure>`.
   - Use the same `figure-panel` pattern for every embedded image. The template script will wrap each `.figure-panel img` with an `image-zoom` trigger and open an in-page `image-lightbox` when clicked.
